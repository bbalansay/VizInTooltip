...
function makeScatterPlot(csvData) {
    ...
    fdata = csvData.filter((data) => {return data.fertility != "NA" && data.life_expectancy != "NA" && data.year == 1980 })

    // get arrays of fertility rate data and life Expectancy data
    let fertility_rate_data = fdata.map((row) => parseFloat(row["fertility"]));
    let life_expectancy_data = fdata.map((row) => parseFloat(row["life_expectancy"]));
...
function plotPopulation(country, toolChart) {
    ...
    let population = countryData.map((row) => parseInt(row["population"]) / 1000000.0);
    ...
    toolChart.append("path")
        ...
        .attr("d", d3.line()
                    ...
                    .y(function(d) { return mapFunctions.yScale(d.population / 1000000.0) }))
